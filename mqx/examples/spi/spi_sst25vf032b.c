...
#include <mqx.h>
#include <bsp.h>
#include "spi_sst25vf032b.h"

#ifdef BSP_SPI_MEMORY_SST25VF032B
...
void memory_set_protection (MQX_FILE_PTR spifd, bool protect)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        buffer[0] = SPI_MEMORY_WRITE_LATCH_STATUS;
...
void memory_write_byte (MQX_FILE_PTR spifd, uint32_t addr, unsigned char data)
{
    ...
    buffer[0] = SPI_MEMORY_WRITE_BYTE;
...
uint32_t memory_write_data (MQX_FILE_PTR spifd, uint32_t addr, uint32_t size, unsigned char *data)
{
    ...
    {   
        ...
        while(0x01 & memory_read_status (spifd))
        {
            /* Stop transfer */
            fflush (spifd);
            _time_delay(10);
        ...
        buffer[2]   = data[offset++];
        result      = fwrite (buffer, 1, 3, spifd);
