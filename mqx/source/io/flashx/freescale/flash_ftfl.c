...
)
{
    ...
    if (FTFL_SWAP_UNINIT == current_swap_mode)
    {
        ...
        if( !ftfl_flash_erase_sector (dev_ptr, (void *)(BSPCFG_SWAP_INDICATOR_ADDR) , 0))
        {
            return (FTFL_ERR_ACCERR);
        }
         
        if( !ftfl_flash_erase_sector (dev_ptr, (void *)(FTFL_SWAP_UPPER_INDICATOR_ADDR) , 0))
        {
            return (FTFL_ERR_ACCERR);
        }
