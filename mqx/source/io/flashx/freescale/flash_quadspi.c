    ...
    )
{
    ...
    dev_spec_ptr->FLASH_CSS = dev_spec_init->FLASH_CSS;
    dev_spec_ptr->FLASH_CSH = dev_spec_init->FLASH_CSH;
    ...
    _quadspi_flash_enable_serial_clk(dev_spec_ptr->MODULE_ID);
    dev_spec_ptr->SERIAL_CLK = actual_serial_clk;

    /* initialize quadspi driver */
    if (_quadspi_init_setup(dev_ptr) < 0) {
        _quadspi_flash_disable_serial_clk(dev_spec_ptr->MODULE_ID);
        _mem_free(dev_spec_ptr);
        return FALSE;
    }
    ...
    )
{
    ...
    /* set TCSS and TCSH */
    quadspi_reg_ptr->FLSHCR = (dev_spec_ptr->FLASH_CSS << QuadSPI_FLSHCR_TCSS_SHIFT)
        | (dev_spec_ptr->FLASH_CSH << QuadSPI_FLSHCR_TCSH_SHIFT);
