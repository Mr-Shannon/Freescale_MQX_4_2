...
{ /* Body */
    ...
    switch (cmd)
    {
        ...
        case IO_IOCTL_ESAI_VPORT_GET_TX_STATUS:
        {
            esai_fifo_node_get_tx_buffer_status(esai_fifo_node_ptr,
                (ESAI_BUFFER_STATUS *)param_ptr);
            break;    
        }
        case IO_IOCTL_ESAI_VPORT_GET_RX_STATUS:
        {
            esai_fifo_node_get_rx_buffer_status(esai_fifo_node_ptr,
                (ESAI_BUFFER_STATUS *)param_ptr);
            break;    
        }
        case IO_IOCTL_ESAI_VPORT_UPDATE_TX_STATUS:
        {
            esai_fifo_node_update_tx_status(esai_fifo_node_ptr,
                *(uint32_t*)param_ptr);
            break;
        }
        case IO_IOCTL_ESAI_VPORT_UPDATE_RX_STATUS:
        {
            esai_fifo_node_update_rx_status(esai_fifo_node_ptr,
                *(uint32_t *)param_ptr);
            break;
        }
        case IO_IOCTL_ESAI_VPORT_WAIT_TX_EVENT:
        {
            esai_fifo_node_wait_tx_event(esai_fifo_node_ptr,  
                *(uint32_t *)param_ptr);
            break;
        }
        case IO_IOCTL_ESAI_VPORT_WAIT_RX_EVENT:
        {
            esai_fifo_node_wait_rx_event(esai_fifo_node_ptr,  
                *(uint32_t *)param_ptr);
            break;
        }
        default:
