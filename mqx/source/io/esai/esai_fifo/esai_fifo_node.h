...
*/
#define ESAI_NODE_TX_WR_INVALID_PARAMETER           (0x16)

/*
** Error code means ESAI node TX write fails, due to invalid TX state
*/
#define ESAI_NODE_TX_WR_INVALID_STATE               (0x17)

/*
** Error code means ESAI node TX write fails, due to no memory avaliable
*/
#define ESAI_NODE_TX_WR_MEMORY_OUT                  (0x18)

/*
** Error code means ESAI node TX write fails, due to invalid parameter
*/
#define ESAI_NODE_RX_WR_INVALID_PARAMETER           (0x19)

/*
** Error code means ESAI node TX write fails, due to invalid TX state
*/
#define ESAI_NODE_RX_WR_INVALID_STATE               (0x1A)

/*
** Error code means ESAI node TX write fails, due to no memory avaliable
*/
#define ESAI_NODE_RX_WR_MEMORY_OUT                  (0x1B)
...
typedef struct esai_buffer_status
{
...
extern int32_t esai_fifo_node_get_tx_buffer_status(
    ESAI_FIFO_AUD_IO_NODE_STRUCT_PTR node, ESAI_BUFFER_STATUS *status);

extern int32_t esai_fifo_node_get_rx_buffer_status(
    ESAI_FIFO_AUD_IO_NODE_STRUCT_PTR node, ESAI_BUFFER_STATUS *status);

extern int32_t esai_fifo_node_update_tx_status(
    ESAI_FIFO_AUD_IO_NODE_STRUCT_PTR node, uint32_t size);

extern int32_t esai_fifo_node_update_rx_status(
    ESAI_FIFO_AUD_IO_NODE_STRUCT_PTR node, uint32_t size);

extern int32_t esai_fifo_node_wait_tx_event(
    ESAI_FIFO_AUD_IO_NODE_STRUCT_PTR node, uint32_t blocks);

extern int32_t esai_fifo_node_wait_rx_event(
    ESAI_FIFO_AUD_IO_NODE_STRUCT_PTR node, uint32_t blocks);
