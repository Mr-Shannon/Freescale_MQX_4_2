...
)
{
    ...
    ret = _esai_fifo_tx_write_buffer_install(esai_fifo_node);
    if (ret != ESAI_NODE_OK)
    {
    ...
    for (i=0; i<ESAI_TX_DMA_ZERO_PACKET_NUM; i++) {
        ...
        if (esai_fifo_node->asrc_enable)
        {
           esai_fifo_node->tx_dma_zero_packets[i].DST_ADDR =
                (uint32_t)((uint32_t *)&(asrc_regs->ASRD[0].ASRDI));
        }
        else
        {
            esai_fifo_node->tx_dma_zero_packets[i].DST_ADDR =
    ...
    if (esai_fifo_node->asrc_enable) {
        ...
        for (i = 0; i < ESAI_MAX_TX_DMA_PACKET_NUM; i++)
        {
            ...
            /* Default LOOP count */
            esai_fifo_node->tx_dma_packets[i].LOOP_BYTES = 
                esai_fifo_node->tx_size_per_request;
            esai_fifo_node->tx_dma_packets[i].LOOP_COUNT = 
                esai_fifo_node->tx_packet_size/esai_fifo_node->tx_size_per_request;
        }
    ...
    ESAI_FIFO_DEBUG("[ESAI] Start the asrc P2P then, tr_mask = 0x%x\n", tr_mask);
        ...
        if (esai_fifo_node->asrc_enable) {
            ...
            esai_fifo_node->tx_asrc_dma_packet.SRC_ADDR =
                            (uint32_t)((uint32_t *)&(asrc_regs->ASRD[0].ASRDO));
...
)
{
    ...
    {
        esai_fifo_node->tx_error_code = IO_ERROR_WRITE;
        return ESAI_NODE_RX_WR_MEMORY_OUT;
    }
    ...
    for (i = 0; i < ESAI_MAX_RX_DMA_PACKET_NUM; i++)
    {
        ...
        esai_fifo_node->rx_dma_packets[i].LOOP_BYTES = 
            esai_fifo_node->rx_size_per_request;
        esai_fifo_node->rx_dma_packets[i].LOOP_COUNT = 
            esai_fifo_node->rx_packet_size/esai_fifo_node->rx_size_per_request;
    }
...
)
{
    ...
    _lwsem_create(&esai_fifo_node->rx_sem, 0);
...
int32_t esai_fifo_node_get_tx_buffer_status
(
    ESAI_FIFO_AUD_IO_NODE_STRUCT_PTR node,
...
{
    status->block_number = node->tx_packet_num;
    status->block_size = node->tx_packet_size;
    status->buffer_end = (uint8_t *)node->tx_buffer + status->block_number * status->block_size;
    status->buffer_start = node->tx_buffer;
    status->full_block = node->tx_packet_count;
    status->empty_block = status->block_number - status->full_block;
    status->hw_addr = (uint8_t *)node->tx_buffer + (node->tx_in * node->tx_packet_size);
    return MQX_OK;
}

int32_t esai_fifo_node_get_rx_buffer_status
(
    ESAI_FIFO_AUD_IO_NODE_STRUCT_PTR node,
...
{
    status->block_number = node->rx_packet_num;
    status->block_size = node->rx_packet_size;
    status->buffer_end = (uint8_t *)node->rx_buffer + status->block_number * status->block_size;
    status->buffer_start = node->rx_buffer;
    status->full_block = node->rx_packet_count;
    status->empty_block = status->block_number - status->full_block;
    status->hw_addr = (uint8_t *)node->rx_buffer + (node->rx_out * node->rx_packet_size);
    return MQX_OK;
}

int32_t esai_fifo_node_update_tx_status
(
    ESAI_FIFO_AUD_IO_NODE_STRUCT_PTR node,
...
{   
    /* TODO: While the size is smaller than the block size. */
    uint32_t blocks = size/node->tx_packet_size;
    if (NULL == node) 
    {
        node->tx_error_code = IO_ERROR_WRITE;
        return ESAI_NODE_TX_WR_INVALID_PARAMETER;
    }

    if (node->tx_state != ESAI_STATE_RUN) 
    {
        node->tx_error_code = IO_ERROR_DEVICE_INVALID;
        return ESAI_NODE_TX_WR_INVALID_STATE;
    }
    /* If the first time, install the buffer. */
    node->write_times++;
    node->tx_buffer_window += blocks;
    node->tx_packet_count += blocks;
    /* Change the tx in index */
    node->tx_in = (node->tx_in + blocks) % node->tx_packet_num;
    return MQX_OK;
}

int32_t esai_fifo_node_update_rx_status
(
    ESAI_FIFO_AUD_IO_NODE_STRUCT_PTR node,
...
{
    /* TODO: While the size is smaller than the block size. */
    uint32_t blocks = size/node->rx_packet_size;
    if (NULL == node) 
    {
        node->tx_error_code = IO_ERROR_WRITE;
        return ESAI_NODE_TX_WR_INVALID_PARAMETER;
    ...
    if (node->rx_state != ESAI_STATE_RUN) 
    {
        node->rx_error_code = IO_ERROR_DEVICE_INVALID;
        return ESAI_NODE_TX_WR_INVALID_STATE;
    }
    /* If the first time, install the buffer. */
    node->read_times ++;
    if (node->rx_packet_count >= blocks)
    {
        node->rx_packet_count -= blocks;
    }
    else
    {
        node->rx_packet_count = 0;
    ...
    /* Change the tx in index */
    node->rx_out = (node->rx_out + blocks) % node->rx_packet_num;
    return MQX_OK;
}

int32_t esai_fifo_node_wait_tx_event
(
    ESAI_FIFO_AUD_IO_NODE_STRUCT_PTR node,
...
{
    uint32_t time_out = node->tx_timeout;
    uint32_t i  = 0;
    MQX_TICK_STRUCT start_tick, end_tick;
    int32_t tick_diff, retval;
    bool overflow, poll;

    for (i = 0; i < blocks; i ++ )
    {
        /* Handle the timeout if setting this */
        if (node->tx_timeout > 0)
        {
            if (time_out > 0)
            {
                _time_get_ticks(&start_tick);
                /*try to get the semaphore, may block the calling task*/
                retval = _lwsem_wait_ticks(&(node->tx_sem), time_out);
                if (MQX_LWSEM_WAIT_TIMEOUT == retval) {
                    /*set time out error code*/
                    node->tx_error_code = IO_ERROR_TIMEOUT;
                    return retval;
                }
                ...
                if (overflow || (uint32_t)tick_diff > time_out)
                    time_out = 0;
                else
                    time_out = time_out - tick_diff;

            } 
            else 
            {
                /*use non-blocking semaphore get when timeout becomes 0*/
                poll = _lwsem_poll(&(node->tx_sem));
                if (!poll) 
                {
                    /*set time out error code*/
                    node->tx_error_code = IO_ERROR_TIMEOUT;
...
int32_t esai_fifo_node_wait_rx_event
(
    ESAI_FIFO_AUD_IO_NODE_STRUCT_PTR node,
...
{
    uint32_t time_out = node->rx_timeout;
    uint32_t i  = 0;
    MQX_TICK_STRUCT start_tick, end_tick;
    int32_t tick_diff, retval;
    bool overflow, poll;
    ...
    {
        /* Handle the timeout if setting this */
        if (node->rx_timeout > 0)
        {
            if (time_out > 0)
            {
                _time_get_ticks(&start_tick);
                /*try to get the semaphore, may block the calling task*/
                retval = _lwsem_wait_ticks(&(node->rx_sem), time_out);
                if (MQX_LWSEM_WAIT_TIMEOUT == retval) {
                    /*set time out error code*/
                    node->tx_error_code = IO_ERROR_TIMEOUT;
                    return retval;
