...
const PIT_TIMER_INIT_STRUCT _bsp_timers[MCF5441_NUM_TIMERS] = {
...
{
  volatile MCF5441_STRUCT      *reg_ptr = _PSP_GET_IPSBAR();
  if (timer >= MCF5441_NUM_TIMERS) return NULL;
...
{
  if (timer >= MCF5441_NUM_TIMERS) return 0;
