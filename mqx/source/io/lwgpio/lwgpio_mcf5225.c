...
static const uint8_t port_shift[]={
    0, 8, 24, 16,
    0, 0, 8, 0,
    0, 0, 0, 18,
    1, 0, 20, 16,
    ...
    if (((LWGPIO_PIN_FROM_ID(handle->flags)) == 2) && handle->port_idx==PORTAS)    /* PAS2 exception */
        ...
        /* PSSRL */
            ...
            {
                volatile uint32_t *pcr = (volatile uint32_t *)(gpio_ptr[REG_PCR]+8);
            ...
            {
                volatile uint32_t *pcr = (volatile uint32_t *)(gpio_ptr[REG_PCR]+16);
    ...
    /* PSSR */
        if (attribute_id == LWGPIO_ATTR_SLEW_RATE) 
        {
            volatile uint32_t *pcr = (volatile uint32_t *)(gpio_ptr[REG_PCR]);
    ...
    /* PDSR */
        ...
        {
            volatile uint32_t *pcr = (volatile uint32_t *)(gpio_ptr[REG_PCR]+4);
    ...
    /* PSRRH */
        if (attribute_id == LWGPIO_ATTR_SLEW_RATE) 
        {
            volatile uint16_t *pcr = (volatile uint16_t *)(gpio_ptr[REG_PCR]+14);
    ...
    /* PDSRH */
        ...
        {
            volatile uint16_t *pcr = (volatile uint16_t *)(gpio_ptr[REG_PCR]+22);
...
)
{
    switch (handle->port_idx) {
        ...
        case PORTDD: /* 8-bit PAR registers, standard */
            {
                volatile uint8_t *par = gpio_ptr[REG_PAR] + handle->port_idx;
        ...
        case PORTUC: /* 8-bit PAR registers with quad-function */
            {
                ...
                uint8_t functionmask = (function & 0x03) << (2 * LWGPIO_PIN_FROM_ID(handle->flags));
        ...
        case PORTQS: /* 16-bit PAR registers with quad-function */
            {
                volatile uint16_t *par = (volatile uint16_t *) (gpio_ptr[REG_PAR] + handle->port_idx);
                /* Create temp. new special pinmask for 16-bit registers */
                uint16_t pinmask = 0x03 << (2 * LWGPIO_PIN_FROM_ID(handle->flags));
                uint16_t functionmask = (function & 0x03) << (2 * LWGPIO_PIN_FROM_ID(handle->flags));
        ...
        case PORTQS: /* 16-bit PAR registers with quad-function */
            {
                ...
                uint16_t pinmask = 0x03 << (2 * LWGPIO_PIN_FROM_ID(handle->flags));
                /* Get 16-bit PAR value for specific port, mask to get 2-bits for specific pin */
                uint16_t parval = *(volatile uint16_t *)(gpio_ptr[REG_PAR] + handle->port_idx);
        ...
        case PORTTH: /* 16-bit PAR register with quad-funcition residing outside PAR area */
            {
                ...
                uint16_t pinmask = 0x03 << (2 * LWGPIO_PIN_FROM_ID(handle->flags));
                /* Get 16-bit PAR value for specific port, mask to get 2-bits for specific pin */
                uint16_t parval = ((VMCF5225_STRUCT_PTR)_PSP_GET_IPSBAR())->GPIO.PTHPAR;
