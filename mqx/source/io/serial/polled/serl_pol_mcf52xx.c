...
extern bool _mcf52xx_uart_serial_polled_status(MCF52XX_UART_SERIAL_INFO_STRUCT_PTR);
extern uint32_t _mcf52xx_uart_serial_polled_ioctl(MCF52XX_UART_SERIAL_INFO_STRUCT_PTR, uint32_t,
   uint32_t *);
extern uint32_t _mcf52xx_uart_serial_polled_init(MCF52XX_UART_SERIAL_INIT_STRUCT_PTR,
   void   **, char *);
extern uint32_t _mcf52xx_uart_serial_polled_deinit(MCF52XX_UART_SERIAL_INIT_STRUCT_PTR,
...
uint32_t _mcf52xx_uart_serial_polled_install
   (
...
{ /* Body */
  ...
  return _io_serial_polled_install(identifier,
      (uint32_t (_CODE_PTR_)(void *, void **, char *))_mcf52xx_uart_serial_polled_init,
      (uint32_t (_CODE_PTR_)(void *, void *))_mcf52xx_uart_serial_polled_deinit,
      (char    (_CODE_PTR_)(void *))_mcf52xx_uart_serial_polled_getc,
      (void    (_CODE_PTR_)(void *, char))_mcf52xx_uart_serial_polled_putc,
      (bool (_CODE_PTR_)(void *))_mcf52xx_uart_serial_polled_status,
      (uint32_t (_CODE_PTR_)(void *, uint32_t, void *))_mcf52xx_uart_serial_polled_ioctl,
...
uint32_t _mcf52xx_uart_serial_polled_init
   (
...
{ /* Body */
   volatile MCF52XX_UART_STRUCT       *uart_ptr;
   MCF52XX_UART_SERIAL_INFO_STRUCT      *uart_info_ptr;
   uint32_t                             channel;
   uint32_t                             baud;
   ...
   *io_info_ptr_ptr = uart_info_ptr;
   ...
   baud = (uint32_t)((uart_info_ptr->INIT.CLOCK_SPEED + 16 * uart_info_ptr->INIT.BAUD_RATE )/ (32 * uart_info_ptr->INIT.BAUD_RATE));   
   uart_ptr->WRITE.UBG1 = (unsigned char)((baud >> 8) & 0xFF);
   uart_ptr->WRITE.UBG2 = (unsigned char)(baud & 0xFF);

   /* If in multi-drop mode disable RX */
   if ((uart_info_ptr->INIT.UMR1_VALUE & MCF52XX_UART_UMR1_MULTI_DROP) == MCF52XX_UART_UMR1_MULTI_DROP){
      /* Disable RX, enable TX */
      uart_ptr->WRITE.UCR = MCF52XX_UART_UCR_TX_ENABLE |
                           MCF52XX_UART_UCR_RX_DISABLE;
   } else { /* Not in multi-drop mode*/
      /* Enable RX & TX */
      uart_ptr->WRITE.UCR = MCF52XX_UART_UCR_TX_ENABLE |
...
uint32_t _mcf52xx_uart_serial_polled_deinit
   (
      /* [IN] the initialization information for the device being opened */
      MCF52XX_UART_SERIAL_INIT_STRUCT_PTR io_init_ptr,
...
{ /* Body */
   volatile MCF52XX_UART_STRUCT      *uart_ptr = io_info_ptr->UART_PTR;
   bool data = FALSE;
   ...
   while ( !data ){
      ...
      /* Multi drop mode */   
      if ( ((io_info_ptr->INIT.UMR1_VALUE & MCF52XX_UART_UMR1_MULTI_DROP) == MCF52XX_UART_UMR1_MULTI_DROP) &&       
         (uart_ptr->READ.USR & MCF52XX_UART_USR_PE) ) { 
         /*input character is address*/
         if ( (unsigned char )io_info_ptr->INIT.STATION_ADDR == uart_ptr->READ.URB ){
            /* Enable RX */
            uart_ptr->WRITE.UCR &= ~MCF52XX_UART_UCR_RX_DISABLE;
            uart_ptr->WRITE.UCR |= MCF52XX_UART_UCR_RX_ENABLE;
         } else {
            /* Disable RX */
            uart_ptr->WRITE.UCR &= ~MCF52XX_UART_UCR_RX_ENABLE;
            uart_ptr->WRITE.UCR |= MCF52XX_UART_UCR_RX_DISABLE;
...
bool _mcf52xx_uart_serial_polled_status
   (
      /* [IN] the address of the device specific information */
      MCF52XX_UART_SERIAL_INFO_STRUCT_PTR io_info_ptr
   )
...
uint32_t _mcf52xx_uart_serial_polled_ioctl
   (
      /* [IN] the address of the device specific information */
      MCF52XX_UART_SERIAL_INFO_STRUCT_PTR io_info_ptr,
...
{ /* Body */
   ...
   switch (cmd) {
      ...
      case IO_IOCTL_SERIAL_SET_BAUD:
         ...
         /* baud = fsck / (32 * baudrate) + 0.5 */ 
         baud = (uint32_t)((io_info_ptr->INIT.CLOCK_SPEED + 16 * io_info_ptr->INIT.BAUD_RATE )/ (32 * io_info_ptr->INIT.BAUD_RATE));
         uart_ptr->WRITE.UBG1 = (unsigned char)((baud >> 8) & 0xFF);
         uart_ptr->WRITE.UBG2 = (unsigned char)(baud & 0xFF);
         break;
      ...
      case IO_IOCTL_SERIAL_GET_PARITY:
         switch (io_info_ptr->INIT.UMR1_VALUE & MCF52XX_UART_UMR1_MD_ADDRESS) {
            ...
            case MCF52XX_UART_UMR1_HI_PARITY:
               ...
               break;
            case MCF52XX_UART_UMR1_MULTI_DROP:
               tmp = IO_SERIAL_PARITY_MULTI_DATA;
               break;
            case MCF52XX_UART_UMR1_MD_ADDRESS:
      ...
      {
         MCF52XX_UART_SERIAL_INIT_STRUCT_PTR io_init_ptr = (MCF52XX_UART_SERIAL_INIT_STRUCT_PTR)param_ptr;
