...
static void (_CODE_PTR_ old_isr_timer)(void *);
static void            *old_isr_timer_param;


/* Declarations of static functions */
static void _adt_scheduler(_mqx_uint time);
static void _adt_hw_set_wait(uint32_t time);
static uint32_t _adt_hw_get_time();
static _mqx_int _adt_hw_run();
static _mqx_int _adt_hw_stop();

/* Declarations of IRQ handlers */
static void adt_setup_timer(uint32_t);
static void adt_cpu_isr(void *);
...
** TIMER_CONST >> (12 + 32 - i)
*/
#define TIMER_CONST (uint64_t)(0xF424000000000000 / (BSP_SYSTEM_CLOCK / 2))
...
*
*END*********************************************************************/
_mqx_int _adt_hw_ioctl(MQX_FILE_PTR f, _mqx_uint cmd, void *param_ptr)
{
...
*
*END*********************************************************************/
_mqx_int _adt_hw_install(char *drivername, ADC_DRIVER_BUNDLE_PTR adc_d_bundle, void *init_struct)
{
...
{ /* Body */
    ...
    _mqx_int         i;
    uint32_t          frq_system_kHz;
    uint16_t          prescaler, preset;
    uint32_t          result;
    ...
    **/
    result = (uint32_t)(TIMER_CONST >> (12 + 32 - 31));
...
{ /* Body */
    ...
    */
    adt_ch->g.period = (((uint64_t) adt_ch->g.period << 31) / (TIMER_CONST >> (12 + 32 - 31))) >> adt_internal.adc_d_bundle->adt->shift; /* note: we are losing bits for timer setting */;
    adt_ch->g.offset = (((uint64_t) adt_ch->g.offset << 31) / (TIMER_CONST >> (12 + 32 - 31))) >> adt_internal.adc_d_bundle->adt->shift; /* note: we are losing bits for timer setting */;
...
* Returned Value   : uint32_t - actual absolute time
* Comments         : 
*
*END**********************************************************************/
static uint32_t _adt_hw_get_time()
...
*
*END**********************************************************************/
static void adt_setup_timer(uint32_t time)
{
    uint32_t  j;
    _mqx_int i;
    uint16_t  preset, prescaler;
