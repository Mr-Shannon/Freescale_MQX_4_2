...
{ /* Body */
    _mqx_int    i, j;
    uint16_t *results = (void *) &mcf522xx_adc_ptr->ADRSLT[0]; /* prepare pointer to peripheral results */
...
{
#if PSP_MQX_CPU_IS_MCF5225X
    return (void *)(&((VMCF5225_STRUCT_PTR)_PSP_GET_IPSBAR())->ADC);
#elif PSP_MQX_CPU_IS_MCF5222X
    return (void *)(&((VMCF5222_STRUCT_PTR)_PSP_GET_IPSBAR())->ADC);
#elif PSP_MQX_CPU_IS_MCF5223X
    return (void *)(&((VMCF5223_STRUCT_PTR)_PSP_GET_IPSBAR())->ADC);
#else
    #error This CPU is not supported here
    return NULL;
#endif
