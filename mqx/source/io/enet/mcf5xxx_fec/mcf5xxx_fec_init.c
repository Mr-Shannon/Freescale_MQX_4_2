...
uint32_t MCF5XXX_FEC_initialize
   (
...
{ 
   ...
   */
   ...
   _mem_set_type((void *)fec_context_ptr, MEM_TYPE_IO_ENET_MAC_CONTEXT_STRUCT);
   
   enet_ptr->MAC_CONTEXT_PTR = (void *) fec_context_ptr;  
...
// TODO remake to using alloc_align fn
   ...
   fec_context_ptr->UNALIGNED_RING_PTR = (void *) bd_ptr;
   fec_context_ptr->FEC_RX_RING_PTR = (VMCF5XXX_FEC_BD_STRUCT_PTR) MCF5XXX_FEC_BD_ALIGN((uint32_t)bd_ptr);
   ...
   _mem_set_type((void *)fec_context_ptr->TxPCBS_PTR, MEM_TYPE_IO_PCB_PTR);
   ...
   _mem_set_type((void *)fec_context_ptr->RX_PCB_BASE, MEM_TYPE_IO_PCB_STRUCT);
   ...
   for (i=0;i<enet_ptr->PARAM_PTR->NUM_SMALL_BUFFERS;i++) {
      ENET_Enqueue_Buffer((void **)&fec_context_ptr->SMALL_BUFFERS, buf_ptr);
      buf_ptr += MCF5XXX_SMALL_PACKET_SIZE;
   }

   // Queue large packets on large buffer Q.
   for (i=0;i<enet_ptr->PARAM_PTR->NUM_LARGE_BUFFERS;i++) {
      ENET_Enqueue_Buffer((void **)&fec_context_ptr->LARGE_BUFFERS, buf_ptr);
   ...
   fec_ptr->PALR = *(uint32_t *) &enet_ptr->ADDRESS[0];
   fec_ptr->PAUR = (*(uint16_t *)(&enet_ptr->ADDRESS[4])) << 16;
   ...
   fec_ptr->ERDSR = (uint32_t)(fec_context_ptr->FEC_RX_RING_PTR);
   fec_ptr->ETDSR = (uint32_t)(fec_context_ptr->FEC_TX_RING_PTR);
   ...
   _mem_zero((void *)fec_ptr->MIB_RAM, sizeof(fec_ptr->MIB_RAM));
   fec_ptr->MIBC = 0;
