...
{ 
   ...
   MCF5XXX_FEC_rx_free_specific(fec_context_ptr, pcb_ptr, (void **)&fec_context_ptr->SMALL_BUFFERS);
}
...
{ 
   ...
   MCF5XXX_FEC_rx_free_specific(fec_context_ptr, pcb_ptr, (void **)&fec_context_ptr->LARGE_BUFFERS);
}
   ...
   */
   FEC_int_disable();
      ...
      for (i=0;pcb_ptr->FRAG[i].FRAGMENT;i++) {
         ENET_Enqueue_Buffer((void **)&fec_context_ptr->RX_BUFFERS, pcb_ptr->FRAG[i].FRAGMENT );
         pcb_ptr->FRAG[i].FRAGMENT = NULL;
      }
   #else   
      ENET_Enqueue_Buffer((void **)&fec_context_ptr->RX_BUFFERS, pcb_ptr->FRAG[0].FRAGMENT );
      pcb_ptr->FRAG[0].FRAGMENT = NULL;
   #endif
...
{ 
   ...
   */
   ...
   while (fec_context_ptr->ActiveRxBDs < fec_context_ptr->NumRxBDs) {
      ...
      bd_ptr->BUFFER = ENET_Dequeue_Buffer((void **) &fec_context_ptr->RX_BUFFERS);
      ...
      */
      ...
      _DCACHE_FLUSH_MBYTES((void *)bd_ptr, sizeof(MCF5XXX_FEC_BD_STRUCT));  
...
static void MCF5XXX_FEC_discard_current_rx(ENET_CONTEXT_STRUCT_PTR    enet_ptr,MCF5XXX_FEC_CONTEXT_STRUCT_PTR   fec_context_ptr, bool last)
{
   uint32_t  i;
   ...
   if (fec_context_ptr->CurrentRxECB) {
      for (i=0;i<fec_context_ptr->CurrentRxFrag;i++) {
         ...
         if (fec_context_ptr->FRAGS[i].FRAGMENT) {
            ENET_Enqueue_Buffer((void **)&fec_context_ptr->RX_BUFFERS, fec_context_ptr->FRAGS[i].FRAGMENT);
         }
...
{ /* Body */
   ...
   while (fec_context_ptr->ActiveRxBDs > 0) {
         ...
         if (fec_context_ptr->CurrentRxECB ) {
            ...
            if (!last) { 
               ...
               if (pcb_ptr) {
                  ...
                  if (!first) {
                     // Coalese
                     large_packet = ENET_Dequeue_Buffer((void **)&fec_context_ptr->LARGE_BUFFERS);
                  ...
                  // Check to see if it is a small packet
                     ...
                     if (length < MCF5XXX_SMALL_PACKET_SIZE) {
                        ...
                        small_packet = ENET_Dequeue_Buffer((void **)&fec_context_ptr->SMALL_BUFFERS);
      ...
      if (!consumed) {
         ENET_INC_STATS(COMMON.ST_RX_DISCARDED);
         ENET_Enqueue_Buffer((void **)&fec_context_ptr->RX_BUFFERS, buffer);
      }
   }
   MCF5XXX_FEC_add_buffers_to_rx_ring(fec_context_ptr);
} 
