...
* Comments:
*
*  This file contains definitions that belongs to the ksz8081 PHY chip
*
*
*END************************************************************************/
...
/*FUNCTION*-------------------------------------------------------------
*
*  Function Name  : phy_ksz8081_discover_addr
*  Returned Value : none
*  Comments       :
...
static bool phy_ksz8081_discover_addr
   (
...
static uint32_t phy_ksz8081_get_speed
   (
       ENET_CONTEXT_STRUCT_PTR     enet_ptr
   )
{ 
   ...
   if ((*enet_ptr->PARAM_PTR->ENET_IF->MAC_IF->PHY_READ)(enet_ptr, PHY_MII_REG_PHYC1, &speed_status, MII_TIMEOUT)) {
      ...
      if ((PHY_MII_REG_PHYC1_OP_MODE_10_HD==speed_status) || (PHY_MII_REG_PHYC1_OP_MODE_10_FD==speed_status)) {
         return 10;
      } else if ((PHY_MII_REG_PHYC1_OP_MODE_100_HD==speed_status) || (PHY_MII_REG_PHYC1_OP_MODE_100_FD==speed_status)) {
         return 100;
      }
   }
   return 0;
...
static bool phy_ksz8081_get_link_status
   (
       ENET_CONTEXT_STRUCT_PTR     enet_ptr
   )
{ 
