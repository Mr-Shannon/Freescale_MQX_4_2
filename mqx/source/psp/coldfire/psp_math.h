...
/* Macro to add a single uint32_t to an array of uint_16s */
#define PSP_ADD_ELEMENT_TO_ARRAY(a_ptr, v, r_ptr, size) \
   _psp_add_element_to_array((uint16_t *)((void *)(a_ptr)), \
      (uint16_t)(v), \
      (uint16_t *)((void *)(r_ptr)), \
      (_mqx_uint)(size))
...
extern uint64_t _psp_mul_32_32(uint32_t, uint32_t);
extern uint32_t _psp_div_64_32(uint64_t, uint32_t);
