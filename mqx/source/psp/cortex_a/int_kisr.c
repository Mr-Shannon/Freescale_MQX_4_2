...
)
{
    ...
    INT_KERNEL_ISR_FPTR old = NULL;
    uint32_t result;
    extern uint32_t reset_addr[];
    ...
    if (A5_FIRQ >= vector) {
        old = (INT_KERNEL_ISR_FPTR)reset_addr[vector];
        reset_addr[vector] = (uint32_t)isr_ptr;
        result = MQX_OK;
    }
    else {
    ...
    _KLOGX3(KLOG_int_install_kernel_isr, old, result);
    return (old);
#else
    return NULL;
#endif
