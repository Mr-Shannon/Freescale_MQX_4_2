   ...
   )
{
    ...
    if (16 <= irq && irq >= PSP_INT_FIRST_INTERNAL && irq <= PSP_INT_LAST_INTERNAL) {
        // set priority
...
/*!
 * \brief Invokes specifed interrupt.
 *
 * \param irq
 *
 * \return _mqx_uint MQX_OK or error code
 */
_mqx_uint _gic_swint_invoke(_mqx_uint irq) {
    _mqx_uint gic_irq_no = irq - 16;

    if (irq >= PSP_INT_FIRST_INTERNAL && irq <= PSP_INT_LAST_INTERNAL) {              
         GICD_ISPENDR(gic_irq_no / 32) = (1 << (gic_irq_no % 32));      
    }
    else
        return MQX_INVALID_PARAMETER;
